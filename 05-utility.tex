...
\cmnt{
  ...
  In the final section I should introduce the most fine-grained
  bearers of utility as ``rewards'' or ``outcomes'', like I do in the
  beginning of ch.7.  That's importance to avoid confusion in the next
  chapter.
...
The general point is that by using `utility' to measure whatever the
...
We will have a brief look at the weighing of different considerations
...
\begin{exercise}
  ...
  \cmnt{%
    ...
    Students didn't get this. Not obvious what the states are etc.
...
All that said, the model we are studying has applications in which the
utilities do not represent the agent's desires, or in which the
credences do not represent her beliefs. For example, the board of
...
not sure whether a given choice would end up being good or bad. Here
the utility function would be derived from the relevant ethical system
...
\begin{exercise}
  ...
  \cmnt{%
    ...
    Students didn't get this.

    Clearer: if you know your future self will want two beers if you
    have one, and otherwise prefers one over none like you, here's the
    utilities: no beer 0, 1 beer 1, 2 beers 2. Note if you don't have
    a beer this is not the actual future U function! How in general is
...
That does not look plausible. Intuitively, if something is certain to
...
The example also indicates where your basic desires are hiding in your
