...
a norm of rationality. It is also hard to see why the sensation should guide an
...
This suggests that the bet is worth £$x$: £$x$ is the most you should
pay for the bet.
...
\begin{genericthm}{The betting interpretation}
  ...
  unit bet on $A$ for up to £$x$ (and she would sell a unit bet on $A$ for £$x$
  or more).
\end{genericthm}
%
...
either £2 or £1, and guaranteed to be at least £1. Again, the argument generalizes
...
shows that an agent with probabilistic credences can't be ``Dutch booked'' with a
...
There's a second reason why rational agents don't always value bets by their
expected payoff, not even if their subjective utility is adequately measured by
...
``topic-neutral'': they should hold for beliefs of every kind, not just for
...
second. Whenever the agent is equally confident in two propositions, they are
...
It's easy to see that if there is one such assignment then there are
...
order to have degrees of belief at all. Here is the
