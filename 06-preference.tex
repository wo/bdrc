...
simpler version of it in the 1944 monograph \emph{Theory of Games and Economic
  Behavior}, co-authored with Oskar Morgenstern. This work is widely taken to
...
supposed to represent the extent to which the agent, on balance, wants the
...
conventional, are said to have a \textbf{ratio scale}. That's because even though the
...
\begin{align*}
  ...
  A \succsim B & \Leftrightarrow \text{$A \succ B$ or $A \sim B$.}
\end{align*}
%
...
\begin{exercise}{2}
  If our agent is indifferent between all rewards,  the procedure stalls at
...
identified by the method. But the method is not guaranteed to work. Besides, it
does not settle how the agent evaluates the options in decision problems in which the
...
\begin{genericthm}{Reduction (of Compound Lotteries)}
  If $L_1$ and $L_2$ are two (possibly compound) lotteries that lead
...
that $\Cr$ is unique and $\U$ is unique except for the choice of zero and unit.
...
for $\Cr$ to ``make sense'' of the agent's preferences, it must conform to the
...
second-most influential book in the history of decision theory, after \emph{Theory
  of Games and Economic Behavior}. I won't go through Savage's method and axioms.
...
the number is odd'. If you thought you'd face the bizarre choice between these options, your beliefs
...
than $B$. The problems just reviewed suggest that utilities and credences aren't
easily derived from preferences in the first sense. But they might be derivable
from preferences in the more theoretical second sense, perhaps following the
techniques of von Neumann and Ramsey.

This would still be valuable. We might still get an interesting argument for
...
\begin{sources}
  ...
  tradition of Ramsey, Savage, and von Neumann and Morgenstern is Peter
  ...
  chapter 3 of Daniel M.\ Hausman, \emph{Preference, Value, Choice, and Welfare}
  (2012), and
