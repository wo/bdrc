...
\cmnt{%
  ...
  I need to explain what I mean by ``is unique except...'' in the
  theorems.
...
In session \ref{ch:probabilism}, I emphasized that we shouldn't think
...
\begin{exercise}
  ...
  \cmnt{%
    Need to clarify that EU is not U!
  } %
\end{exercise}
...
would get $a$ with probability \nicefrac{4}{5} and $b$ with
probability \nicefrac{1}{5}. Since the expected utility of this gamble
is \nicefrac{1}{5}, we could then infer that the agent's utility for $c$
is \nicefrac{1}{5}.
...
\cmnt{% 
  I need to clarify that 1/2 is only an example.
} %
...
\begin{genericthm}{Completeness}
  For any $A$ and $B$, either $A \succ B$ or $B \succ A$ or $A \sim B$.
\end{genericthm}
\cmnt{%
...
In session \ref{ch:probabilism} we looked at the betting
...
and Savage in session \ref{ch:risk}. You may already get a glimpse of
them if you consider how the model of utility from the previous
session meshes with the accounts of von Neumann and Morgenstern and
Savage. In the previous session, we saw how an agent's utility
...
\cmnt{%
  ...
  In the previous session, we saw how an agent's utility function is
