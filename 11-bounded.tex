...
depends on the agent's utility function. This suggests that we might reduce the
...
\cmnt{%
  ...
  the shop was not strictly speaking an available act, since it was
...
$A_1,\ldots,A_{50}$, we only need to store $2^{20}$ values. Second, if our robot's
...
need to store from $100^{10}$ to 90,100.%
\cmnt{%
...
and Amos Tversky in the 1970s-1990s.
...
is white. The other is either white or red. You have no opinion about the
...
the monetary payoff. The permissivist account then classifies both bets as
...
\begin{sources}
  ...
  by Seamus Bradley (2019) provides a good overview of research on the topic of
