...
flat in the suburbs for £600/month. You might draw up a list of pros and cons
for each option, and give them a score, like so:
...
\end{center}
\medskip\noindent%
You might then determine the \emph{total} utility of each option as the sum of
...
flats are then determined by your preferences between combinations of these
...
\]
Here, there are two attributes, each of which can take two values. The first
...
for different degrees of pain, the first attribute would have more than two
...
prefer a central \mbox{40 $\m^2$} flat for £500 to a central 60 $\m^2$ flat for £800,
then we have
\[
...
subvalues she assigns to the individual parts of the stream. That is, if $p_1$,
...
\]
where $\delta$ is a fixed number greater than 0. The interesting thing here is
...
much to the agent as one unit today; the day after tomorrow it is worth a
quarter; and so on.
...
$\t{0, 0, 0, 0, 11, 0, 0, \ldots}$ to $\t{0, 0, 0, 10, 0, 0, 0, \ldots}$. These
...
not represent your basic desires, or not all of them. If, for example, you have a
...
to which a proposition satisfies your present cravings, and another subvalue
...
\begin{sources}
  ...
  O'Donoghue, ``Time Discounting and Time Preference: A Critical Review''
  (2002).

  A simple version of Harsanyi's proof of utilitarianism is given in Michael D.\
